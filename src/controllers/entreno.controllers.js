import views from '../views/entreno.html'
import '../css/entreno.css'
import '../source/checkbox.css'
...
export default () =>{
    ...
    ( function () {
    const input = divElement.querySelector('#itnew');
    const form = divElement.querySelector('#formitnew');
    const selectList = divElement.querySelector('#slist');
    const listsContainer = divElement.querySelector('#lists');
    //* funcion uuidv4
    ...
    function uuidv4() {
      return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,(c) => {const r = (Math.random() * 16) || 0; 
    ...
    function renderTodos() {
      ...
      const todosContainer = divElement.querySelector('#todos');
      ...
      divElement.querySelectorAll('.todo label input').forEach((item) => {
      ...
      divElement.querySelectorAll('.todo button').forEach((item) => {
    ...
    function validardscga(e) {
      const $boton = divElement.querySelector('#btnCrearPdf');
      $boton.addEventListener('click', () => {
        const $elementoParaConvertir = divElement.querySelector('#todos'); // ?<-- recuerden que como el elemento se genera dinamicamente no puedo seleccionar el document
        html2pdf()
          .set({
            margin: 1,
...
// ? funcion para validar si el evento key se ejecuta
    $(document).ready(function(e) {
      ...
      // ? busco que el usuario presione enter para enviar la informacion y que se active la opcion de descarga
       const eventoEnter = input.addEventListener('keypress', (e) => {
        const contenido = divElement.querySelector('.todo');
          const campo = divElement.querySelector('#itnew');
          const text = campo.value.trim();
          if (e.key === 'Enter' && !text == '' && contenido == null) {
            validardscga(e);
          }
        });
